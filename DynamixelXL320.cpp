...
void DynamixelXL320::sendSyncWriteU16Packet(int id_size, int address, int values[]){
  unsigned int parameter_data_size = 4 + id_size*3;
...
DynamixelXL320::DxlV2_Packet::DxlV2_Packet(unsigned char *data, size_t data_size, unsigned char id, unsigned char instruction, int parameter_data_size, ...) {
  ...
  unsigned short crc = XL320_update_crc(0,this->data,getSize()-2);
...
bool DynamixelXL320::DxlV2_Packet::isValid() {
  ...
  return storedChecksum == XL320_update_crc(0, this->data, length+5);
}
